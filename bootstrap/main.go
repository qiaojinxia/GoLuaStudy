...
func printHeader (f *binchunk.Prototype) {
	...
	fmt.Printf("\n%s <%s:%d,%d> (%d instructions) \n",funcType,
		f.Source,f.LineDefined,f.LastLineDefined,len(f.Code))
	fmt.Printf("%d%s params,%d slots ,slots %d upvalues, ",f.NumParams,
		varargFlag,f.MaxStackSize,len(f.UpvalueNames))
...
func constantOString(k interface{})string {
	switch k.(type) {
	case nil: return "nil"
